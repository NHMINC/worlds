...
- **Documented simplifications** (decreed, not hidden): metallic core and
  ...
  (1/1000) inside the object's 0.5 ly sphere, and `ARRIVE_K · d`
  ...
  fly out of its 0.5 ly sphere — a look drag only releases the
  ...
  for flying the disk; inside the 0.5 ly sphere that gain falls
...
- **Explorer gestures.** The camera stays at the bubble centre.
  ...
  0.5 ly sphere (fixed, not object-size) every move is under
