...
- **The explorer is the luminous harvest, not a magnifier ball.**
  ...
  **point sources**: a 1px Teff core plus the eye’s PSF.
  Apparent magnitude is `max(MIN, FLOOR + GAIN · (L / d²)^P)`
  with no bright-end cap — MIN holds the field up (2× the
  reference pin); stars already brighter are left alone. Magnitude lifts the wings; it does not stamp a
