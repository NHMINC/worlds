...
check(starKind(asObj(freshWd)) === 6, `planetary nebula should draw as a shell, got ${starKind(asObj(freshWd))}`);
{
  ...
  check(pin <= 7, `harvest-floor star must stay a small pin, got ${pin.toFixed(2)}px`);
  ...
  check(shineFloor > 0.45 && shineFloor < 0.8, `floor core must be a visible pin, got ${shineFloor.toFixed(2)}`);
  ...
  check(floorWing < 0.08, `floor PSF must stay a pin, wing ${floorWing.toFixed(3)}`);
