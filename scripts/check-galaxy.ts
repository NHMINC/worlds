...
check(starKind(asObj(freshWd)) === 6, `planetary nebula should draw as a shell, got ${starKind(asObj(freshWd))}`);
{
  ...
  check(shineFloor < 0.25, `floor core must stay a pin, got ${shineFloor.toFixed(2)}`);
  ...
  check(harvestShine(400, 2) > harvestShine(400, 12), 'same L must dim with distance (apparent mag)');
