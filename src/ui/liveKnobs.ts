...
export const LIVE_KNOBS: LiveKnob[] = [
  ...
  },
  {
    ...
    about: 'Ahead only. Full disk warp outside this radius. Inside it, speed falls as Close crawl × distance so you meet the sphere speed limit at the fence — no dump onto the shell, no skip through it. Astern does not use this: sphere limit, then warp. 50 ly is the law (about two warp frames). Next frame.',
  ...
  },
  {
    ...
    about: 'Distance, not a clock. Ahead: full at the 50 ly brake, this floor at the fill park, linear in between — same span as the slowdown. Astern: the host sphere only, then full survey light. 0.08 is a dark-sky Earth night, a bit clearer — the only galaxy light on anything in the bubble. 0 is pitch black. The host furnace is not multiplied. Next frame.',
    uniform: 'uArriveSky',
    min: 0,
    max: 0.4,
    step: 0.005,
