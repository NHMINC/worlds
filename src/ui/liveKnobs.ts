...
export const LIVE_KNOBS: LiveKnob[] = [
  ...
  },
  {
    ...
    label: 'Abyss density',
    group: 'dust',
    hint: 'crest density where one cloud goes lightless',
    about: 'Per-cloud opacity: each cloud a sightline crosses ramps from the dust floor up to this crest density, so most clouds are translucent and only a rare dense crest is an abyss on its own. Overlapping clouds add — stacks can still reach black. Lower makes more abysses; higher makes all dust hazier. Stars and the cosmic background obey the same sum. Next frame.',
