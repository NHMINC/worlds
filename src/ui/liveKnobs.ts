...
export const LIVE_KNOBS: LiveKnob[] = [
  ...
  },
  {
    ...
    about: 'Full disk warp outside this radius. Inside it, speed falls as Close crawl × distance so you meet the sphere speed limit at the fence — no dump onto the shell, no skip through it. The same curve in reverse on the way out. 1 ly is the law. Next frame.',
  ...
  },
  {
    ...
    about: 'Distance, not a clock. The enhanced photograph is full at the approach brake (1 ly) and this floor at the fill park, linear in between — same span as the slowdown, so dimming does not wait for the sphere. Leaving is the reverse. 0.08 is a dark-sky Earth night, a bit clearer — the only galaxy light on anything in the bubble. 0 is pitch black. The host furnace is not multiplied. Next frame.',
    uniform: 'uArriveSky',
    min: 0,
    max: 0.4,
    step: 0.005,
  ...
  },
  {
    ...
    about: 'Extra cap inside the host bubble (warp, WASD, strafe, zoom), on top of Close crawl × distance. The 1 ly brake already meets the fence at Close crawl × fence, so this only binds if you raise the sphere or the crawl. Next frame.',
    uniform: 'uArriveWarp',
    min: 0.0002,
    max: 0.02,
    step: 0.0002,
