...
export const REBUILD_KNOBS: RebuildKnob[] = [
  ...
  },
  {
    ...
    label: 'Arm contrast',
    group: 'dust',
    hint: 'how much dust prefers the spiral arms',
    about: 'Mild arm hint on the photograph. Low keeps many small clumps along the limbs; high pulls them onto one spiral ridge (the snake). Needs rebake.',
    scope: 'dust',
    key: 'GALAXY_DUST_ARM',
    min: 0,
    max: 2,
    step: 0.05,
    read: () => UNIVERSE.GALAXY_DUST_ARM,
    write: (v) => writeNum('GALAXY_DUST_ARM', v),
  },
  {
    id: 'denseCut',
    label: 'Clump floor',
    group: 'dust',
    hint: 'field threshold for a dark cloud',
    about: 'ismAt.photo below this does not write. Lower = more, smaller clouds; higher = fewer, darker patches. Needs rebake.',
    scope: 'dust',
  ...
  {
    id: 'dustStreak',
    label: 'Clump contrast',
    group: 'dust',
    hint: 'how hard each cloud’s core is',
    about: 'Power on the dense-tail excess. Higher punches the cores and thins the edges. Needs rebake.',
    scope: 'dust',
  ...
  {
    id: 'dustShear',
    label: 'Filament stretch',
    group: 'dust',
    hint: '1 = round ticks; higher = longer dashes',
    about: 'Along/across aspect of each photograph filament in the disk. 1 is a round blob (vertical tick edge-on). A little over 2 is a short dash. The occupancy 4.2 shear was one galaxy-length snake. Needs rebake.',
    scope: 'dust',
    key: 'GALAXY_DUST_SHEAR',
    min: 0.4,
    max: 4,
    step: 0.05,
    read: () => UNIVERSE.GALAXY_DUST_SHEAR,
    write: (v) => writeNum('GALAXY_DUST_SHEAR', v),
  },
  {
    id: 'dustTurn',
    label: 'Filament turn',
    group: 'dust',
    hint: 'how fast the filament angle wanders',
    about: 'Cycles per kpc of the envelopes that pick among five in-plane filament angles. Higher and neighbouring clouds point different ways. Lower and a patch shares one angle. Needs rebake.',
    scope: 'dust',
    key: 'GALAXY_DUST_TURN',
    min: 0.1,
    max: 2,
    step: 0.02,
    read: () => UNIVERSE.GALAXY_DUST_TURN,
    write: (v) => writeNum('GALAXY_DUST_TURN', v),
  },
  {
    id: 'dustTilt',
    label: 'Filament tilt',
    group: 'dust',
    hint: 'how much a filament leans out of the disk',
    about: 'Radians of lean off the midplane. 0 is perfectly laid down (horizontal dashes edge-on). A little tilt turns the side-on view into slashes at mixed angles instead of a picket fence. Needs rebake.',
    scope: 'dust',
    key: 'GALAXY_DUST_TILT',
    min: 0,
    max: 1.2,
    step: 0.02,
    read: () => UNIVERSE.GALAXY_DUST_TILT,
    write: (v) => writeNum('GALAXY_DUST_TILT', v),
  },
];
